...
public class ApplicationMRDomain extends BaseAPIDomain{
	...
	protected CBSAccessLog mapping(DBCollection dbColl,String mappingUrl,String serviceName,DBObject query,int totalSize) {
...
//			double totalHits=getTotalHits();
			...
			double ninetyPercentLine=generate90Line(totalHits,mappingUrl,query);
			
			CBSAccessLog cbsAccessLog=new CBSAccessLog();
			cbsAccessLog.setUrl(mappingUrl);
	...
	private double getMax(){
		...
		return getMax.getMapReduceResult(dbColl,mappingUrl, serviceName,query);
	}
	
	private double getMin(){
		Aggregation_Min getMin=new Aggregation_Min(dbColl);
		return getMin.getMapReduceResult(dbColl,mappingUrl, serviceName,query);
	}
	...
	private double generate90Line(Double totalHits, String apiName,DBObject query) {
		if (totalHits != null && totalHits > 10) {
			...
			query.put("mappingUrl",
					java.util.regex.Pattern.compile("" + mappingUrl + ""));
			int numToLimit = (int) (totalHits * 0.9);
			int numToSkip = (int) (totalHits - numToLimit);
			int batchSize = 1;
			DBCursor dbCur = dbColl
					.find(new BasicDBObject().append("mappingUrl", apiName))
