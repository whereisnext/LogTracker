...
public class Aggregation_ErrorCount implements MapReduce {
	...
	public Aggregation_ErrorCount(DBCollection dbColl) {
		this.dbColl=dbColl;
	}
	...
	@Override
	public void setMap() {
		this.map="function Map(){emit('status',this.status)}";
		
	}

	@Override
	public void setReduce() {
		this.reduce="function (key, values) { var count = 0; values.forEach(function (val) { if (val !=200 && val !=304) count = count + 1; }) " +
				"\n"+
				"return count}";
...
//				queryBuilder.put("status");
				DBObject statusCode=new BasicDBObject();
				statusCode.put("status", 200);
				statusCode.put("status", 304);
				
				queryBuilder.notEquals(statusCode);
