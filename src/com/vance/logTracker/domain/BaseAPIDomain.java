...
public abstract class BaseAPIDomain {
...
//		MongodbInstanceFactory.connect(url);
		if(db==null||dbColl==null){
		db= MongodbInstanceFactory.getDBInstance(dbName);
	...
	protected CBSAccessLog mapping(MapReduceCommand cmd, DBCollection dbColl,
		...
		CBSAccessLog aggregationedAccessLog=cacheProcess(cbsAccessLogs,size);
...
//		aggregationedAccessLog.setAvgResponseDataSize(aggregationedAccessLog.getResponseDataSize()/size);
	...
	private CBSAccessLog cacheProcess(ArrayList<CBSAccessLog> cbsAccessLogs,int logsCount) {
		...
		for (CBSAccessLog cbsAccessLog : cbsAccessLogs) {
			if(api==""){
				api=cbsAccessLog.getApi().trim();
			}else if(!api.equals(cbsAccessLog.getApi())){
				
				System.out.println("Something wrong....."+cbsAccessLog.getApi().trim());
			}
...
//			totalHits += cbsAccessLog.getTotalHits();
		...
		result.setTotalHits(totalHits);
		result.setUrl(url);
		result.setNinetyPercentResp(generate90Line(result.getTotalHits(), result.getApi()));
	...
	private int generate90Line(Double totalHits, String apiName) {
		if (totalHits != null && totalHits > 10) {
			...
			DBCursor dbCur = dbColl
					.find(new BasicDBObject().append("action", Pattern
							.compile("(.*)" + apiName + "(.*)",
									Pattern.CASE_INSENSITIVE)))
					.sort(new BasicDBObject("responseTime", -1))
					.limit(numToLimit).skip(numToSkip);
			Iterator<DBObject> it = dbCur.iterator();
			return Integer.valueOf(dbCur.next().get("responseTime").toString());
		} else {
			return -1;
		}
	}
